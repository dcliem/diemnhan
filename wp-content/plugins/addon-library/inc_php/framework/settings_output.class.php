	...
	class UniteSettingsOutputUCWork{
		...
		* set form id
		...
		public function drawWrapperStart(){
			...
			?>
			<div id="<?php echo $this->wrapperID?>" data-options="<?php echo $strOptions?>" autofocus="true" class="unite_settings_wrapper <?php echo $this->settingsMainClass?> unite-settings unite-inputs">
		...
		*/
		...
		public function drawSettings($filterSapID = null){
			...
			foreach($arrSettings as $key=>$setting){
				...
				if($this->showSaps == true && $this->sapsType == self::SAPS_TYPE_CUSTOM)
					$this->drawSaps($setting);
