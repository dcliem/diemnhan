	...
	class UniteSettingsUC{
		...
		private function getOptionsFromXMLField($field,$fieldName){
			...
			foreach($options as $option){
				...
				//validate options:
				UniteFunctionsUC::validateNotEmpty($optionValue,"option value");
		...
		private function addExternalSettings($filename, $loadfrom){
			...
			switch($loadfrom){
				case "helper":
					$filepathSettings = GlobalsUC::$pathHelpersSettings.$filename.".php";
				break;
				case "current":
					$filepathSettings = GlobalsUCGallery::$pathSettings.$filename.".php";
				break;
				default:
					UniteFunctionsUC::throwError("addExternalSettings error: unknown helper foldername: {$loadfrom}");
				break;
			}
			...
			if(!isset($settings))
				UniteFunctionsUC::throwError("The settings include file: {$filepathSettings} must include '\$settings' object.");
