...
class Awsm_drivr
{
    ...
    public function drivr_shortcode($atts)
    {
        $embed     = '';
        $durl      = '';
        $embedsrc  = 'https://drive.google.com/file/d/%s/preview';
        $exporturl = 'https://drive.google.com/uc?export=download&id=%s';
        extract(shortcode_atts(array('id' => '','type'=> 'application'), $atts));

        if (!$id) {
            $embed = sprintf('<div class="ead-preview"><p>%s</p></div>', __('No Url Found', $this->text_domain));
            return $embed;
        }
        $iframesrc    = sprintf($embedsrc, $id);
    ...
    public function drivr_media()
    {
        ...
        wp_localize_script('media_button', 'drivr', $this->getoptions());
    }
    /**
     * Drivr media button
     */
