...
add_action( 'genesis_entry_content', 'dn_do_post_content_nav', 12 );

// filter page links for correct bootstrap format
add_filter( 'wp_link_pages_link', 'dn_wp_link_pages_link' );

function dn_wp_link_pages_link( $link ) {
    if ( $link && '<' !== $link[0] ) {
...
function dn_do_post_content_nav( $attr ) {
    wp_link_pages( array(
