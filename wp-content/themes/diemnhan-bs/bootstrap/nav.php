...
add_filter( 'wp_nav_menu_args', 'dn_nav_menu_args_filter' );

// add bootstrap markup around the nav
add_filter( 'wp_nav_menu', 'dn_nav_menu_markup_filter', 10, 2 );

function dn_nav_menu_args_filter( $args ) {
    ...
    if (
        ...
        $args['walker'] = new wp_bootstrap_nav_walker();
    }

    return $args;
}

function dn_nav_menu_markup_filter( $html, $args ) {
...
EOT;
        ...
        if ( 'primary' === $args->theme_location ) {
            $output .= apply_filters( 'dn_navbar_brand', dn_navbar_brand_markup() );
        }
...
function dn_navbar_brand_markup() {
